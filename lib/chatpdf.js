...
const MODEL_EXP = { '12-2025-chat-model': 'gemini-2-5-flash-sep' };
...
function httpReq(url, { method = 'GET', headers = {}, body, timeout = 120000 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      headers: { 'user-agent': CHROME_UA, 'accept-language': 'fr-FR,fr;q=0.9', 'accept': '*/*',
        'origin': 'https://www.chatpdf.com', 'referer': 'https://www.chatpdf.com/', 'content-type': 'application/json', ...headers },
    }, res => {
      const chunks = [];
...
async function uploadToFirebase(pdfBuffer, { chatId, sourceId } = {}) {
  ...
  const meta = {
    ...
    language: 'fr-FR', userId: 'anon_' + nanoid(24), platform: 'Linux x86_64',
  };
  ...
  return { sourceId: sourceId2, chatId: chatId2, storagePath, stored };
}
...
async function processUpload({ chatId, sourceId, storagePath, filename, doInitChat = true }) {
  const body = JSON.stringify({
    type: 'processUpload', chatId, sourceId, storagePath, doInitChat,
    filename, userInfo: { localTimeHour: new Date().getHours(), fullName: '' },
  });
  const res = await httpReq(`${WEBAPI}/bigstream`, { method: 'POST', body, timeout: 120000 });
...
async function ask({ chatId, history, aiMsgId, userMsgId, msgType = 'standard' }) {
  const aiId = aiMsgId || 'MY' + nanoid(8);
  ...
  const res = await httpReq(`${WEBAPI}/stream`, { method: 'POST', body, timeout: 180000 });
...
async function summarizeDocument({ chatId, history, userMsgId }) {
  const msg = 'Résume ce document de manière structurée et concise, en français.';
  const qId = userMsgId || nanoid(10);
  return ask({ chatId, history: [...history, { id: qId, author: 'u_', type: 'standard', msg, time: Date.now() }], userMsgId: qId });
}

module.exports = { uploadToFirebase, processUpload, ask, summarizeDocument, accountMeta, sourceHighlights, parseReferences, stripMarkup, nanoid, ChatPdfError };
