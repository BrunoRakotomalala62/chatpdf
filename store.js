...
function createChat({ chatId, sourceId, filename, greeting, suggestedQuestions, title, anonId, languageCode }) {
  const id = chatId || 'cha_' + nanoid();
  const chat = {
    id, createdAt: new Date().toISOString(),
    sourceId, filename, title: title || filename || null,
    anonId: anonId || null, languageCode: languageCode || 'fr',
    aiMessageId: null,
    messages: [],
  };
  if (greeting) {
...
function listChats() {
  return Object.values(db.chats).sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || '')).map(c => ({
    id: c.id, filename: c.filename, title: c.title, sourceId: c.sourceId, anonId: c.anonId || null,
